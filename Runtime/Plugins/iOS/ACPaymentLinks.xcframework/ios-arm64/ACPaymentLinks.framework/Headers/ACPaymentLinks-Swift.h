...
@class NSNumber;
SWIFT_ENUM_FWD_DECL(NSInteger, BrowserMode)
@class NSString;
@class ACConfigModel;
...
SWIFT_CLASS_PROPERTY(@property (nonatomic, class, strong) NSNumber * _Nullable setDebugMode;)
+ (NSNumber * _Nullable)setDebugMode SWIFT_WARN_UNUSED_RESULT;
+ (void)setSetDebugMode:(NSNumber * _Nullable)newValue;
...
/// The delegate must conform to <code>ACPaymentLinksDelegate</code> and is used to receive success, failure, or cancellation events during the purchase process.
...
SWIFT_CLASS("_TtC14ACPaymentLinks13LoggerManager")
...
- (nonnull instancetype)init SWIFT_UNAVAILABLE;
+ (nonnull instancetype)new SWIFT_UNAVAILABLE_MSG("-init is unavailable");
- (NSString * _Nonnull)getGlobalTraceId SWIFT_WARN_UNUSED_RESULT;
+ (NSString * _Nonnull)getTraceIdBridge SWIFT_WARN_UNUSED_RESULT;
@end

@interface NSObject (SWIFT_EXTENSION(ACPaymentLinks))
